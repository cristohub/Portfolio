...
const ContactSection: React.FC = () => {
  const [selectedInterest, setSelectedInterest] = useState<string>('Diseño UX/UI');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!email.trim() || !validateEmail(email)) errors.push('Correo electrónico no válido.');
    if (!phone.trim() || !validatePhone(phone)) errors.push('Número de teléfono no válido. Debe tener 9 dígitos.');
    ...
    emailjs.send('service_t5evh0d', 'template_ns9ze8j', templateParams, 'CF8pokcLxmKWP2D2h')
      .then(() => {
        Swal.fire({
          icon: 'success',
        ...
        <div className="row align-items-center text-white">
          ...
          <div className="col-md-6 mb-5 mb-md-0">
            ...
            <p className="d-flex align-items-center mb-3">
              <i className="bi bi-envelope-fill me-3" style={{ fontSize: '1.5rem', color: '#61dafb' }}></i>
              <span>cristofersani04@gmail.com</span>
            </p>
            <p className="d-flex align-items-center mb-3">
              <i className="bi bi-linkedin me-3" style={{ fontSize: '1.5rem', color: '#61dafb' }}></i>
              <a href="https://www.linkedin.com/in/cristofersani" target="_blank" rel="noreferrer" className="text-white text-decoration-none">
                www.linkedin.com/in/cristofersani
              </a>
            </p>
            <p className="d-flex align-items-center mb-3">
              <i className="bi bi-telephone-fill me-3" style={{ fontSize: '1.5rem', color: '#61dafb' }}></i>
                ...
                <div className="d-flex flex-wrap gap-2">
                  {interests.map((interest) => (
                    <button
                      key={interest}
                      type="button"
                      className={`btn btn-sm rounded-pill ${selectedInterest === interest ? 'btn-dark text-white' : 'btn-outline-secondary'}`}
